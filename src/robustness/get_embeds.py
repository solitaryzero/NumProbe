...
def read_dataset(path):
    ...
    full_dataset = {
        'train': {'id': [], 'a': [], 'b': [], 'digit': [], 'golden': [], 'prompt': [], 'template_type': []},
        'val': {'id': [], 'a': [], 'b': [], 'digit': [], 'golden': [], 'prompt': [], 'template_type': []},
        'test': {'id': [], 'a': [], 'b': [], 'digit': [], 'golden': [], 'prompt': [], 'template_type': []},
    }
    with open(path, 'r', encoding='utf-8') as fin:
        for line in fin:
            ...
            for i, template in enumerate(templates):
                ...
                full_dataset[spl]['template_type'].append(i)
            ...
            if (args.save_embeds):
                try:
                    ...
                    for layer_index, tensor in enumerate(t):
                        _tensor = tensor[0, special_positions[1], :].detach().cpu().numpy() # end of num1
                        _tensor = _tensor.reshape((-1))
