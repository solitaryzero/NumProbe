...
def analyze_all_layers(
    ...
    for layer in tqdm(range(args.layer_count)):
        ...
        for j, X in enumerate(all_X):
            ...
            # key_indices = list(range(num1_start)) \
            #             + list(range(num1_end, num2_start)) \
    ...
    beautify(fig)
    # x_labels = fig.get_xticklabels()
    # fig.set_xticklabels([selected_tokens[int(p.get_text())] for p in x_labels])
